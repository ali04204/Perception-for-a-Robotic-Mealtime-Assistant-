...
def main():
    ...
    if len(sys.argv) > 1:
        # Videos given on the command line
        video_paths = [pathlib.Path(p) for p in sys.argv[1:]]
    else:
        # All mp4 videos in data/raw
    ...
    for in_path in video_paths:
        ...
        if out_csv.exists():
            print(f"Skipping {in_path.name} since {out_csv.name} already exists")
            continue
        ...
        with mp_holistic.Holistic(
            ...
            while True:
                ...
                if frame_idx % 50 == 0:
                    print(f"{in_path.name}: processed {frame_idx} frames")
